...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    //get container with getElementById instead of querySelector
    switch(size) {
      case "1":
  ...
  function determineDx (elem, size) {
    //simplify use of variables, replace querySelector with getElementById
  ...
  function changePizzaSizes(size) {
    /*
      - obtain an array of elements before looping named pizzaContainers using getElementsByClassName
...
//code for testing performance. This could be commented out for performance
...
function updatePositions() {
  ...
  //items obtained via getElementsByClassName rather than querySelectorAll
  var i, phases = [],
      scrollExtent = document.body.scrollTop / 1250,
      items = document.getElementsByClassName('mover');

  //there are only 5 phases, calculate values for the phases once
  for (i = 0; i < 5; i++) {
    phases.push(Math.sin(scrollExtent + (i % 5)));
  }
  //index the value of phase from phases array
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (i = 0; i < numPizzas; i++) {
    ...
    //whole integer width makes it easier for browser to render image changed from 73.333px
